...
export class MemoryStore {
    ...
    async bulkStore(entries) {
        ...
        const validEntries = entries.filter((entry) => entry && entry.text && entry.text.length > 0 && entry.vector && entry.vector.length > 0);
        ...
        return this.runWithFileLock(async () => {
            try {
                ...
                const code = err.code || "";
                const message = err.message || String(err);
                throw new Error(`Failed to bulk store ${fullEntries.length} memories: ${code} ${message}`);
            }
            return fullEntries;
        });
    }
