...
describe("shouldCapture — German triggers", () => {
  ...
  it("captures 'Ich will immer Tests vor dem Merge'", () => {
    assert.equal(shouldCapture("Ich will immer Tests vor dem Merge"), true);
  });
  ...
  // --- Emphasis triggers (with word boundaries) ---
  it("captures 'Das ist immer so in Production'", () => {
    assert.equal(shouldCapture("Das ist immer so in Production bei uns"), true);
  });
...
describe("shouldCapture — German false-positive prevention", () => {
  ...
  it("does NOT capture 'Wichtigkeit' (contains 'wichtig' substring)", () => {
