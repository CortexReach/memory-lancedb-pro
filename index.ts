...
/**
 * Phase 2 — Singleton State Interface
 *
 * Tracks all heavy plugin resources that must be initialized exactly once.
 * Session Maps persist across scope refreshes (re-registrations) instead of
 * being recreated each time register() is called, which was causing memory
 * accumulation and resource waste.
 *
 * Phase 1 (_hookEventDedup) handles event deduplication (PR #430 / #617).
 * Phase 2 (_singletonState) handles resource deduplication (this PR).
 * Phase 3 (backup timer optimization) is NOT included.
 */
...
/** Singleton state: null until first register() call, then persists for process lifetime. */
let _singletonState: PluginSingletonState | null = null;

/**
 * Phase 2 — Initialize all heavy plugin resources exactly once.
 *
 * Called by register() when _singletonState is null.
 * All heavy resources and session Maps are created here and stored in
 * module-level _singletonState. Subsequent register() calls reuse the
 * same singleton via destructuring.
 */
