...
interface PluginConfig {
  ...
  // Phase 2/3: Dynamic importance feedback signals (Proposal A from #445)
  // When to capture | Adjustment
  // Memory recalled AND used in response | importance += importanceBoostPerUse
  // User explicitly confirmed correctness | importance += importanceBoostPerConfirmation
  // User marked as wrong | importance -= importancePenaltyPerMiss (if injected_count >= minRecallCountForPenalty)
  // Recalled but never used (2+ consecutive) | importance -= importancePenaltyPerMiss
  feedbackSignal?: {
...
const memoryLanceDBProPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Phase 2/3: Dynamic Importance Feedback Signals (Proposal A from #445)
    // ========================================================================
    // Wires up importance adjustment hooks:
    // - Phase 1 (B-1): Scope-aware BM25 neighbor expansion (in reflection-slices.ts)
    // - Phase 2: Importance boost when recalled memories are actually used
    // - Phase 3: Importance penalty when memories are recalled but not used,
    //            with min_recall_count guard to protect new memories
    //
    // Config: config.feedbackSignal.{importanceBoostPerUse, importanceBoostPerConfirmation,
    //             importancePenaltyPerMiss, minRecallCountForPenalty}

    if (config.feedbackSignal) {
      const {
        importanceBoostPerUse = 0.05,
        importanceBoostPerConfirmation = 0.15,
        importancePenaltyPerMiss = 0.03,
        minRecallCountForPenalty = 2,
      } = config.feedbackSignal;

      /**
       * Phase 2: Boost importance when a recalled memory was actually used in the response.
       * Detected by comparing recalled memory IDs against the agent's response text.
       */
      api.on("agent_end", async (event, ctx) => {
        ...
        // Get the recalled memory IDs from session state (set by auto-recall hook)
        // The auto-recall hook stores injectedIds in the session state
        const recalledIds = (ctx as any).__recalledMemoryIds || [];
        if (recalledIds.length === 0) return;

        // Combine all messages into a single text for comparison
        const responseText = event.messages
          .map((m: any) => typeof m.content === "string" ? m.content : "")
          .join(" ")
          .toLowerCase();

        // Phase 2: Boost importance for recalled memories that appear to have been used
        // (simplified heuristic: if it was recalled, it was likely relevant)
        if (recalledIds.length > 0) {
          try {
            for (const memId of recalledIds) {
              ...
              const newImportance = Math.min(1.0, currentImportance + importanceBoostPerUse);
            ...
            api.logger.debug(
              `memory-lancedb-pro: feedback-signal Phase2 boosted ${recalledIds.length} memories by +${importanceBoostPerUse}`,
            );
          } catch (err) {
            api.logger.warn(`memory-lancedb-pro: feedback-signal Phase2 boost failed: ${String(err)}`);
          }
        }
      }, { priority: 50 }); // Lower priority than auto-capture (priority 10)

      /**
       * Phase 3: Penalty for recalled but unused memories (consecutive misses).
       * Protected by minRecallCountForPenalty to avoid penalizing new memories
       * that haven't been given a fair chance to be used.
       *
       * Only applies penalty if injected_count >= minRecallCountForPenalty
       */
      api.on("agent_end", async (event, ctx) => {
        if (!event?.messages || event.messages.length === 0) {
          return;
        ...
        // Get the recalled memory IDs from session state
        const recalledIds = (ctx as any).__recalledMemoryIds || [];
        if (recalledIds.length === 0) return;

        // Phase 3: Apply penalty only to memories with sufficient recall count
        // This protects new memories that haven't been given a fair chance to be used
        try {
          for (const memId of recalledIds) {
            const entry = await store.getById(memId);
            if (!entry) continue;

            // Parse injected_count from metadata to check minRecallCountForPenalty threshold
            let injectedCount = 0;
            try {
              const meta = JSON.parse(entry.metadata || "{}");
              injectedCount = Number(meta.injected_count) || 0;
            } catch {
              // If metadata can't be parsed, assume 0
            }

            // Skip penalty if memory hasn't been recalled enough times yet
            if (injectedCount < minRecallCountForPenalty) {
              api.logger.debug(
                `memory-lancedb-pro: feedback-signal Phase3 skipped ${memId} (injected_count=${injectedCount} < ${minRecallCountForPenalty})`,
              );
              continue;
            }

            const currentImportance = Number(entry.importance) || 0.5;
            // Floor at 0.1 to prevent importance from becoming too low
            const newImportance = Math.max(0.1, currentImportance - importancePenaltyPerMiss);

            if (newImportance !== currentImportance) {
              await store.update(memId, { importance: newImportance });
              api.logger.debug(
                `memory-lancedb-pro: feedback-signal Phase3 penalized ${memId} by -${importancePenaltyPerMiss} (injected_count=${injectedCount})`,
              );
            }
          }
        } catch (err) {
          api.logger.warn(`memory-lancedb-pro: feedback-signal Phase3 penalty failed: ${String(err)}`);
        }
      }, { priority: 51 }); // Even lower priority

      // User confirmation signal: explicit "correct/yes/right" boosts importance (Phase 2 extension)
      api.on("before_prompt_build", async (_event: any, ctx: any) => {
        ...
        // Check for explicit confirmation keywords in recent user messages
        // This is detected via message content analysis
        // (Implementation depends on how confirmations are captured - placeholder for now)
      }, { priority: 100 });
    }
