  ...
  for (const derivedLine of derived) {
    ...
    try {
      ...
      for (const hit of hits) {
        ...
        // Self-filter: skip if hit text is identical to the query derived line
        // (compare raw derived line to hit text — D4 already truncated if > 120 chars)
        if (text === derivedLine) continue;
